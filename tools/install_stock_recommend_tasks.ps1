...
$openArgs = "-m scheduler.daily_job --mode open_confirm --push$mockArg"
$postArgs = "-m scheduler.daily_job --mode post_market --push$mockArg"
$snapshotArgs = "-m scheduler.daily_job --mode previous_close_snapshot$mockArg"
...
$openAction = New-ScheduledTaskAction -Execute $PythonExe -Argument $openArgs -WorkingDirectory $backend
$postAction = New-ScheduledTaskAction -Execute $PythonExe -Argument $postArgs -WorkingDirectory $backend
$snapshotAction = New-ScheduledTaskAction -Execute $PythonExe -Argument $snapshotArgs -WorkingDirectory $backend
$weeklyAction = New-ScheduledTaskAction -Execute $PythonExe -Argument $weeklyArgs -WorkingDirectory $backend
$preTrigger = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday -At "08:06"
$openTrigger = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday -At "10:00"
$postTrigger = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday -At "18:30"
$snapshotTrigger = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday,Tuesday,Wednesday,Thursday,Friday -At "17:30"
...
Register-ScheduledTask -TaskName "AHAnalyse-OpenConfirm" -Action $openAction -Trigger $openTrigger -Settings $settings -Description "A-share open confirmation of conditional tickets" -Force
...
Register-ScheduledTask -TaskName "AHAnalyse-PreviousCloseSnapshot" -Action $snapshotAction -Trigger $snapshotTrigger -Settings $settings -Description "A-share previous-close full-market snapshot" -Force
...
Write-Host "Installed pre-market, open-confirm, previous-close snapshot, post-market, and weekly reweight tasks."
