...
class AkshareSnapshotProvider:
    ...
    def snapshot(self, limit: int) -> List[Dict[str, Any]]:
        ...
        return frame.replace({pd.NA: None}).to_dict(orient="records")
...
class MarketDataManager:
    def __init__(
        ...
        *,
        ...
        self.order = list(order or ["hithink_financial_api", "akshare"])
        self.breakers = {
        ...
        for name in supplement_names:
            ...
            try:
                ...
                for row in result.rows:
                    ...
                    if not secondary:
                        continue
                    before = {key: row.get(key) for key in SUPPLEMENT_FIELDS}
